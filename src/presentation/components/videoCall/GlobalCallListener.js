...
const GlobalCallListener = () => {
    ...
    useEffect(() => {
        ...
        socket.on(VIDEO_CALL_EVENT.CALL_REQUEST, (data) => {
            const { callerId, callerName } = data;
            toast((t) => (_jsxs("div", { className: "flex flex-col gap-2", children: [_jsxs("p", { className: "font-medium", children: [callerName ?? "Someone", " is calling..."] }), _jsxs("div", { className: "flex gap-2", children: [_jsx("button", { className: "bg-green-500 text-white px-3 py-1 rounded-md text-sm", onClick: () => {
                                    toast.dismiss(t.id);
                                ...
                                }, children: "Accept" }), _jsx("button", { className: "bg-red-500 text-white px-3 py-1 rounded-md text-sm", onClick: () => {
                                    toast.dismiss(t.id);
                                    socket.emit(VIDEO_CALL_EVENT.CALL_REJECT, {
                                        callerId: data.callerId,
                                        calleeId: data.calleeId,
                                ...
                                }, children: "Reject" })] })] })), { duration: 10000 });
            eventBus.emit("CALL_REQUEST", data);
        });
        socket.on(VIDEO_CALL_EVENT.CALL_ACCEPT, (data) => {
            eventBus.emit("CALL_ACCEPT", data);
        });
        socket.on(VIDEO_CALL_EVENT.CALL_REJECT, (data) => {
            eventBus.emit("CALL_REJECT", data);
        });
        socket.on(VIDEO_CALL_EVENT.OFFER, (data) => {
            eventBus.emit("OFFER", data);
        });
        socket.on(VIDEO_CALL_EVENT.ANSWER, (data) => {
            eventBus.emit("ANSWER", data);
        });
        socket.on(VIDEO_CALL_EVENT.ICECANDIDATE, (data) => {
            eventBus.emit("ICECANDIDATE", data);
        });
        socket.on(VIDEO_CALL_EVENT.CALL_END, (data) => {
            eventBus.emit("CALL_END", data);
        });
        return () => {
            socket.off(VIDEO_CALL_EVENT.CALL_REQUEST);
