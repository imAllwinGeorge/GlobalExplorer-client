...
const RevanueChart = ({ salesDetails }) => {
    const data = {
        labels: MONTHS,
        datasets: [
            {
                label: `${currentYear} Sales`,
                data: MONTHS.map((_, i) => salesDetails.current.find((b) => b._id.month === i + 1)?.totalSales || 0),
                backgroundColor: "#4ade80", // green (up)
                borderRadius: 6,
            },
            {
                label: `${currentYear - 1} Sales`,
                data: MONTHS.map((_, i) => -(salesDetails.previous.find((b) => b._id.month === i + 1)?.totalSales || 0)), // make values negative
                backgroundColor: "#f87171", // red (down)
                borderRadius: 6,
            },
        ],
    };
    return (_jsx("div", { className: "h-[400px] w-full", children: _jsx(Bar, { data: data, options: options }) }));
};
export default RevanueChart;
