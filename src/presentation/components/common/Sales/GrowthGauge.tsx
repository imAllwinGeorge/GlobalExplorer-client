...
type GrowthGaugeProps = {
  // 0 - 100
  value: number
  // overall rendered size in px (width and height of the svg box)
  size?: number
  // thickness of each tick line
  thickness?: number
  // number of small segments around the arc
  segments?: number
  // arc start/end angle in degrees (0° is at top, increasing clockwise)
  startAngle?: number
  endAngle?: number
  // center text under the percent
  label?: string
  // small helper text under the gauge
  subtext?: string
  // classes to style the wrapper (e.g., padding)
  className?: string
}
...
export default function GrowthGauge({
  ...
  thickness = 8,
  segments = 36,
  startAngle = 150, // degrees (0 at 12 o'clock)
  ...
  return (
    <>
      <div
        className={["flex w-full flex-col items-center justify-center", "text-foreground", className].join(" ")}
        role="img"
        aria-label={`Growth gauge at ${clamped}%`}
        style={{ ["--gauge-size" as string]: `${size}px` }}
      >
        <svg
          viewBox={`0 0 ${size} ${size}`}
          className="h-auto w-full max-w-[var(--gauge-size)] overflow-visible"
          aria-hidden="true"
        ...
        >
          {ticks.map((t, idx) => (
            <line
              ...
              strokeLinecap="round"
              className={t.isActive ? "text-primary" : "text-primary"}
        ...
        {subtext ? <div className="mt-6 text-sm text-muted-foreground">{subtext}</div> : null}
      </div>
    </>
  )
}
