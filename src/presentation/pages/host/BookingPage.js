...
const BookingPage = () => {
    ...
    const [selected, setSelected] = useState(OPTIONS.booking[0].value);
    useEffect(() => {
        if (!user)
            return;
        const fetchBookingDetails = async () => {
            try {
                setLoading(true);
                const response = await hostService.activityBookings(user?._id, page, 9, searchQuery, selected);
    ...
    return (_jsxs(motion.div, { initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { duration: 0.5 }, className: "min-h-screen bg-gray-50 p-4 md:p-6", children: [_jsxs("div", { className: "max-w-7xl mx-auto", children: [_jsx(SearchBox, { placeholder: "Search for activities.....", onSearch: (query) => setSearchQuery(query) }), _jsx(RadioGroup, { name: "bookings", options: OPTIONS.booking, onChange: setSelected, value: selected }), data && (_jsxs(_Fragment, { children: [_jsx(ReusableTable, { data: data, columns: columns, columnHeaders: columnHeaders, title: "My Bookings", renderCell: (col, row) => {
                                    if (col === "index")
