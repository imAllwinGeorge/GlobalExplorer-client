...
const VideoChat = () => {
    ...
    const [incomingCall, setIncomingCall] = useState(locState?.incomingCall ?? null);
    const [onCall, setOnCall] = useState(false);
    ...
    useEffect(() => {
        ...
        const iceHandler = async (data) => {
            const relevant = data.to === user?._id ||
                data.from === user?._id ||
                data.to === receiverId ||
                data.from === receiverId;
            if (!relevant)
    ...
    return (_jsxs("div", { className: "relative min-h-[60vh] flex flex-col md:flex-row items-center justify-center gap-4 p-4 md:p-6 bg-background text-foreground", children: [!onCall && !incomingCall && (_jsx("button", { onClick: startCall, className: "inline-flex items-center justify-center rounded-md bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-primary/90 transition-colors", children: "Start Call" })), localStream && (_jsx(VideoCall, { stream: localStream, className: `${remoteStream
                    ? "absolute bottom-4 right-4 z-10 w-28 h-40 md:w-1/4 md:h-1/3 rounded-md ring-2 ring-background shadow-lg"
                    : "w-full h-[60vh] rounded-lg"} bg-muted`, isLocal: true })), onCall && remoteStream && (_jsx(VideoCall, { stream: remoteStream, className: "w-full h-[60vh] rounded-lg overflow-hidden", isLocal: false })), onCall && (_jsx("button", { onClick: hangUp, className: "inline-flex items-center justify-center rounded-md bg-red-600 text-white px-4 py-2 text-sm font-medium hover:bg-red-700 transition-colors", children: "Hang Up" }))] }));
};
export default VideoChat;
